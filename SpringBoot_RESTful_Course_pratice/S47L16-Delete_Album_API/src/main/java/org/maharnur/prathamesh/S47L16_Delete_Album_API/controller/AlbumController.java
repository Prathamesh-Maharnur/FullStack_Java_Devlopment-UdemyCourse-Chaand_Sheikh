package org.maharnur.prathamesh.S47L16_Delete_Album_API.controller;
...
import org.maharnur.prathamesh.S47L16_Delete_Album_API.model.Account;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.model.Album;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.model.Photo;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.payload.album.AlbumPayloadDTO;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.payload.album.AlbumViewDTO;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.payload.album.PhotoDTO;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.payload.album.PhotoPayloadDTO;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.payload.album.PhotoViewDTO;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.services.AccountService;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.services.AlbumService;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.services.PhotoService;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.util.AppUtils.AppUtil;
import org.maharnur.prathamesh.S47L16_Delete_Album_API.util.constants.AlbumError;
...
public class AlbumController {
    ...
    public ResponseEntity<String> delete_album(@PathVariable long album_id, Authentication authentication) {
        try {
            String email = authentication.getName();
            Optional<Account> optionalAccount = accountService.findByEmail(email);
            Account acccount = optionalAccount.get();

            Optional<Album> optionalAlbum = albumService.findById(album_id);
            Album album = null;
            if (optionalAlbum.isPresent()) {
                album = optionalAlbum.get();
                if (album.getAccount().getId() != acccount.getId()) {
                    return ResponseEntity.status(HttpStatus.FORBIDDEN).body(null);
                }
            } else {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
