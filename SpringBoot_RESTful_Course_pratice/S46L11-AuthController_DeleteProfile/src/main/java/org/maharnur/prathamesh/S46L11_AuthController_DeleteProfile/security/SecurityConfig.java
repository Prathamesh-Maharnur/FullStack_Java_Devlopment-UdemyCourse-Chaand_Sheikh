        ...
        public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
                http.csrf(csrf -> csrf.ignoringRequestMatchers("/db-console/**"))
                                ...
                                .authorizeHttpRequests(auth -> auth
                                                .requestMatchers("/", "/auth/token", "/auth/users/add",
                                                                "/swagger-ui/**", "/swagger-ui.html/**",
                                                                "/v3/api-docs/**", "/db-console/**", "/error")
                                                .permitAll()
                                                .requestMatchers("/test", "/auth/profile", "/auth/profile/update-password", "/auth/profile/delete").authenticated()
                                                .requestMatchers("/auth/users", "/auth/users/{user_id}/update-authorities").hasAuthority("SCOPE_ADMIN"))
